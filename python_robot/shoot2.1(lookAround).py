...
7.Used new parameter 8.3
8. Comment "backward time constraint"
9. Look left and right to find ball.
...
while not rospy.is_shutdown():
    ...
    # rotateTo2(0)
    '''
    print "Last rotation position:, ",point_control.get_imu()[2]
    anykey = raw_input("============= Press any key to start =========== \n")
    t = time.time()
    value = int(anykey)
    rotateTo2(value)
    print "Time used 1: ", round((time.time()-t)*1000), "ms"
    
    t = time.time()
    rotateTo2(value)
    print "Time used 2: ", round((time.time()-t)*1000), "ms"
    print "Stopped rotation:, ",point_control.get_imu()[2]
    ...
    if carState == state_begin:
        ...
        point_control2.PointMove(0.8, 0.6, 0)    
        # point_control.PointMove(0.1, 0.0, 0)
        # point_control2.PointMove(0.1, -0.1, 0)
    ...
    if carState == state_findBall:
        ...
        # else:      
            ...
            '''
            Add rotate to left and right ones;
            ...
            if len(result_dict["football"])>1:
                targetPointX, targetPointY = result_dict['football']
                point_control2.PointMove(targetPointX-0.3 * 1.732/2 , targetPointY + 0.3 * 0.5, 0) # change to car frame
                carState = state_findBall
            else:       # no ball left, try turn right
                print "Turn right"
                rotateTo2(-30)
                has_rev = False
                picTime = time.time()
                ...
                if len(result_dict["football"])>1:
                    ...
                    # no ball right. So no ball front, left ,right
