        ...
        if isBackward == 0:
            if angle_error > pi/4 or angle_error < -pi/4:
                ...
                if angle_error > pi/8 or angle_error < -pi/8:
                    ...
                    if angle_speed > 0:
                        move_cmd.angular.z = min(angle_speed, 1.0)
                    else:
                        move_cmd.angular.z = max(angle_speed, -1.0)
    ...
    if mov_stage==2 :
        if isBackward == 0:
            ...
            if distance_error > 0.05:
                ...
                if angular_speed > 0:
                    move_cmd.angular.z = min(angular_speed, 1.0)
                else :
                    move_cmd.angular.z = max(angular_speed, -1.0)
