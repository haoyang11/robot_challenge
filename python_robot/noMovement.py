...
def getPicture():
    ...
    global has_rev, det_flag,ask_camera, cv_image
    has_rev = False
    # picTime = time.time()
...
def getPicture_top():
    global has_rev_top, det_flag_top,ask_camera_top, cv_image_top
    has_rev_top = False
...
def image_callback(msg):
    ...
    if det_flag:
        det_flag=False
        np_narray = np.fromstring(msg.data, np.uint8)  # new deleted
...
# rospy.Subscriber("/tb"+tb_name+"/pub_camera",String,image_callback)
