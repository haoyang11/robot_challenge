...
# ballTargetX, ballTargetY = 1.93, 0.50
...
while not rospy.is_shutdown():
    ...
    global ballTargetX, ballTargetY
    if netInitFinished == False:    # wait for net to finish init
        continue

    if carState == state_begin:
        # print "[state] begin. Move to middle point, 1.83,0.5", ballTargetX, ballTargetY
        ...
        # if move to point and rotate to front, use this one.
        '''
        point_control.PointMove(1.83, 0.5, 0)
        rotateTo(0)
        point_control.PointMove(0.6, 0, 0)
        '''

        # go straight line to push ball directlly to the door
        point_control.PointMove(2.4, 0.6, 0)
