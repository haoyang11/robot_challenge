...
def findBoundaryDistance(src):
    ...
    if cv2.countNonZero(bottomBar) < th:
		...
		print "Cannot find boundary Lines..."
		return 0
    ...
    distance = (1.96 * linePosition + 6280)/(linePosition - 252)        # fitting curve
    print "Line position: ", linePosition, ", distance: ", distance
    return distance 
...
def onTrackbar(value):
    ...
    hLow = cv2.getTrackbarPos("H min", "hsvBar")
    hHigh = cv2.getTrackbarPos("H max", "hsvBar")

    thresh = cv2.getTrackbarPos("Threshold", "Result")
    barWidth = cv2.getTrackbarPos("Bar width", "Result")       
    
    
    global g_hLow, g_hHigh, g_bottomBarThresh, g_barWidth
    g_hLow, g_hHigh = hLow, hHigh
    g_bottomBarThresh, g_barWidth = thresh, barWidth
...
#g_sLow, g_sHigh = 240, 255
#g_vLow, g_vHigh = 50, 230
...
if __name__ == '__main__':
    ...
    cv2.createTrackbar("H min", "hsvBar", 40    , 255, onTrackbar);
    cv2.createTrackbar("H max", "hsvBar", 80, 255, onTrackbar);
    

    cv2.createTrackbar("Threshold", "Result", 20, 100, onTrackbar);
    cv2.createTrackbar("Bar width", "Result", 5, 100, onTrackbar);
