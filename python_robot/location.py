...
def ballpostion(balldata,cam_param):
	# balldata=('football', 0.8986098766326904, (10.4346008300781, 450.2826690673828, 26.574583053588867, 124.28572082519531))
	#parameter
	pic_h=480
	pic_w=640
	...
	try:
		...
		if (v-h/2-border)<0 or (v+h/2+border) > pic_h:
			# print "x_w:",x_w
			ballx=x_w
		if ((v+h/2+border)>pic_h and (u-w/2-border)<0) or ((u+w/2+border)>pic_w and (v+h/2+border)>pic_h):
			ballx=ballx/3
	except:
		ballx,bally=[0,0]
	if ballx>2:
	...
	if abs(bally)>1:
		bally=1
	print "x_w,x_h,ball_ratio",x_w,x_h,ball_ratio
	if ball_ratio<0.1:
		return []
	# part of ball)
...
def doorpostion(doordata,cam_param):
	...
	[w,h]=[coord[2],coord[3]]
	line_h=[18819.10677983,-7.77055584]
...
def allposition(data_list,cam_param):
	...
	# 		('football', 0.8986098766326904, (465.4346008300781, 229.2826690673828, 26.574583053588867, 124.28572082519531))]
