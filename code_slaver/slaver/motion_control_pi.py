...
def PointMove(x,y,z=0):
    ...
    while True:
		...
		#print "<error> angle error:", angle_error

                angle_error = piRange(targetAngle_inBaseFrame - rotation)
		print "targetAgle:", targetAngle_inBaseFrame, ", rotation:", rotation, ", error: ", angle_error
		    ...
		    print "In rotation, angular_speed: ", angular_speed
                    if angular_speed > 0:
...
def get_odom():
    ...
    if use_tf==True:
        ...
        get_rpy=get_imu()
        return (get_trans,get_rpy[2]*pi/180)
...
def test():
    ...
    # PointMove(goal_x, goal_y, goal_z)            pub_point_state.publish(False)               pub_point_state.publish(False)
        ...
        rate.sleep()
         pub_point_state.publish(False)
            PointMove(point_goal[0],point_goal[1],point_goal[2])
            pub_point_state.publish(True)            pub_point_state.publish(False)
            PointMove(point_goal[0],point_goal[1],point_goal[2])
            pub_point_state.publish(True)
        rate.sleep()            pub_point_state.publish(False)
            PointMove(point_goal[0],point_goal[1],point_goal[2])            pub_p            pub_point_state.publish(False)
            PointMove(point_goal[0],point_goal[1],point_goal[2])
            pub_point_state.publish(True)
        rate.sleep()            pub_point_state.publish(False)
            PointMove(point_goal[0],point_goal[1],point_goal[2])
        ...
        rate.sleep()
            ...
            pub_point_state.publish(True)
        rate.sleep()
oint_state.publish(False)            pub_point_state.publish(False)
            PointMove(point_goal[0],point_goal[1],point_goal[2])
            pub_point_state.publish(True)
        rate.sleep()
