...
def recvpic(host,shape=(480,640,3)):
    ...
    while True:
        ...
        try:
            ...
            order = "Pic Request"
            sendlen = 0
            while sendlen < len(order):
                curlen = s.sendto(order[sendlen:],host)
                sendlen += curlen
            print "Pic Request to",host
            im = ""
            while len(im) < shape[0]*shape[1]*shape[2]:
                data,addr= s.recvfrom(10240)
                if addr == host:
                    print "recv from correct addr.",
                    im = im+data
                    print "curlen = ",len(im)
                else:
                    print "recv from false addr"
...
while not rospy.is_shutdown():
    ...
    if carState == state_begin:
        ...
        t = time.time()
        img = getPicture()
        print "TTTTTTTTTTime used by bottom camera:", (int)((time.time()-t)*1000)," ms."
        # img = recvpic(('192.168.1.65',9999))
        ...
        if carState == state_changePosition:
            ...
            t = time.time()
            img = getPicture_top()
            print "TTTTTTTTTTime used by TOP camera:", (int)((time.time()-t)*1000)," ms."
